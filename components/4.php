...
<h1 class="profileName mt-4 text-center">The Hummingbird</h1>
...
<?php 
		...
		function getAllies1() {
			$sql = "
				...
				WHERE relationships.type_id = 1 AND (relationships.hero1_id= 4);
			";
			$request = pg_query(getDb(), $sql);
			return pg_fetch_all($request);
		}
			...
			$sql = "
				...
				JOIN relationship_types ON relationships.type_id = relationship_types.id
				WHERE relationships.type_id = 1 AND (relationships.hero2_id= 4);
			";
		...
		function getEnemies1() {
			$sql = "
				...
				JOIN heroes ON heroes.id = relationships.hero2_id 
				JOIN relationship_types ON relationships.type_id = relationship_types.id
				WHERE relationships.type_id = 2 AND (relationships.hero1_id= 4);"
				;
			...
			$sql = "
				...
				JOIN heroes ON heroes.id = relationships.hero1_id 
				JOIN relationship_types ON relationships.type_id = relationship_types.id
				WHERE relationships.type_id = 2 AND (relationships.hero2_id= 4);
			";
			$request = pg_query(getDb(), $sql);
			return pg_fetch_all($request);
		}
